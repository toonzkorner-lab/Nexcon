...
function tokenIdentityKey(token: string): string {
  ...
  if (payload) {
    try {
      ...
      // Intentionally ignored: a token whose payload is not valid base64url
      // JSON (or has no usable claims) simply falls through to hashing the
      // full token below, which is still a stable identity key.
    }
  }
  return createHash("sha256").update(token).digest("base64url");
}
