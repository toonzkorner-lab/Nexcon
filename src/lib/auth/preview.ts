 ...
 *
 * The value may be overridden with a `PREVIEW_CLIENT_SECRET` env var so the
 * constant can be rotated without a code change. The platform does NOT inject
 * env into the live preview, so there it falls back to the baked value below.
 * Accepted risk: anyone with repo access holds this secret. Rotation procedure:
 * regenerate the broker's `GROK_PREVIEW_CLIENT_SECRET`, then set the matching
 * `PREVIEW_CLIENT_SECRET` env (or update the baked fallback) and redeploy —
 * sessions minted with the old secret stop working.
 */
export const PREVIEW_CLIENT_ID = "grok_preview";
export const PREVIEW_CLIENT_SECRET =
  env("PREVIEW_CLIENT_SECRET") ??
