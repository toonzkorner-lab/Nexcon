...
// The workspace ships a real baked identity (src/lib/og/site.json plus
// public/og.jpg), which the injector prefers over every fallback. Tests that
// verify the fallback chain (document title -> host slug -> appName) or the
// placeholder card must run against an empty workspace, or they test the
// site's identity instead of the fallback logic.
function emptyWorkspace() {
  return mkdtempSync(join(tmpdir(), "grok-pwa-blank-"));
}
