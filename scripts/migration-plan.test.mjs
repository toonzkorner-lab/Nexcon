...
test("the auth schema ships outside the globbed directory", () => {
  // This workspace has sign-in on, so the auth-on copy step has already copied
  // migrations/auth/0001_auth.sql up into the globbed directory. The source of
  // truth still ships outside the glob; the copy's byte-identity is asserted
  // by the test below.
