...
//#endregion
//#region server/middleware/pwa-headers.ts
async function pwaHeaders(event, next) {
	const result = await next();
	...
	} else headers.set("content-type", "application/manifest+json; charset=utf-8");
...
async function securityHeaders(event, next) {
	const result = await next();
	if (!(result instanceof Response)) return result;
	const headers = new Headers(result.headers);
...
var _lazy_RkIcva = defineLazyEventHandler(() => import("./_chunks/ssr-renderer.mjs"));
var findRoute = /* @__PURE__ */ (() => {
	const data = {
		route: "/**",
		handler: _lazy_RkIcva
	};
	return ((_m, p) => {
		return {
			data,
...
var globalMiddleware = [
	...
	toEventHandler(pwaHeaders),
	toEventHandler(securityHeaders)
].filter(Boolean);
//#endregion
