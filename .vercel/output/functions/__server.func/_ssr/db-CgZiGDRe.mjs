...
import { o as getRequest, s as __exportAll$1 } from "./ssr.mjs";
...
import { $t as getCurrentAdapter, A as base64Url, At as array, C as runWithEndpointContext, D as serializeSignedCookie, E as serializeCookie, Ft as number, It as object, Jt as ATTR_HOOK_TYPE, Kt as withSpan, Lt as optional, M as decodeProtectedHeader, Mt as email, N as importJWK, O as filterOutputFields, P as jwtVerify, Pt as looseObject, Qt as getAuthTables, Rt as record, S as getCurrentAuthContext, Sn as defineErrorCodes, T as toResponse, Xt as import_src, Yt as ATTR_OPERATION_ID, Zt as safeJSONParse, _ as createAuthMiddleware, _n as isTest, a as betterFetch, an as initGetFieldName, b as hasRequestState, bn as kAPIErrorHeaderSymbol, c as applyDefaultAccessTokenExpiry, cn as createLogger, d as findInvalidTrustedProxies, en as queueAfterTransactionHook, f as getIp, fn as env, g as createAuthEndpoint, gn as isProduction, h as normalizePathname, hn as isDevelopment, ht as JWTExpired, i as refreshAccessToken, in as initGetModelName, j as decodeJwt, jt as boolean, kt as any, l as isLoopbackHost, ln as logger, n as socialProviders, nn as runWithTransaction, on as generateId, p as deprecate, qt as ATTR_CONTEXT, r as validateAuthorizationCode, rn as getBetterAuthVersion, rt as encode, s as createAuthorizationURL, sn as createRandomStringGenerator, t as SocialProviderListEnum, tn as runWithAdapter, u as createRateLimitKey, un as shouldPublishLog, v as isAPIError, vn as APIError, w as createRouter$1, x as runWithRequestState, xn as BASE_ERROR_CODES, y as defineRequestState, yn as BetterAuthError, zt as string } from "../_libs/@better-auth/core+[...].mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/db-CgZiGDRe.js
var db_CgZiGDRe_exports = /* @__PURE__ */ __exportAll({
	A: () => serializeCommands,
	B: () => getBaseURL,
	C: () => seedSite,
	D: () => parseCommands,
	...
	H: () => formatUsd,
	I: () => GROK_PROVIDERS,
	L: () => hasGateSessionMarker,
	M: () => serializePostBody,
	N: () => slugify,
	O: () => parseMetrics,
	P: () => verify_server_exports,
...
async function createPgliteSql() {
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({
			...
			"/migrations/0002_cms.sql": _0002_cms_default,
			"/migrations/0003_seo.sql": _0003_seo_default,
			"/migrations/0004_bots.sql": _0004_bots_default,
			"/migrations/0005_images.sql": _0005_images_default
		});
...
*/
function ownerEmail() {
	return env$2("OWNER_EMAIL")?.toLowerCase();
}
...
var PREVIEW_CLIENT_SECRET = env$2("PREVIEW_CLIENT_SECRET") ?? "8bcdb7fc5a33874ad933ca568918d5790388a0795e44c4d1dea691f801b17ec5";
/**
...
var grokClientSecret = env$1("GROK_AUTH_CLIENT_SECRET") ?? PREVIEW_CLIENT_SECRET;
...
var verify_server_exports = /* @__PURE__ */ __exportAll$1({
...
function asPostBody(value) {
	...
	for (const item of value) {
		...
		blocks.push({
			heading: row.heading ? String(row.heading) : void 0,
			paragraphs: paragraphs.length ? paragraphs : [""]
		});
	}
	return blocks;
}
function asOrderItems(value) {
	if (!Array.isArray(value)) {
		if (typeof value === "string") try {
...
function slugify(input) {
	return input.normalize("NFKD").replace(/['’]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80);
}
function lines(text) {
...
function serializeMetrics(rows) {
...
function parsePostBody(text) {
	...
	for (const chunk of chunks) {
		...
		if (trimmed.startsWith("# ")) {
			...
			if (last && !last.heading && last.paragraphs.length) last.paragraphs.push(trimmed);
			else blocks.push({ paragraphs: [trimmed] });
		}
	}
	return blocks.length ? blocks : [{ paragraphs: [""] }];
}
function serializePostBody(blocks) {
	return blocks.map((b) => {
...
function serializeCommands(rows) {
...
var STACK_NOTES = [
	{
		name: "React",
		use: "Sites, apps, this studio OS"
	},
	{
		name: "TypeScript",
		use: "The default. Fewer 3 a.m. surprises"
	},
	{
		name: "Node.js",
		use: "APIs, workers, Discord.js, Telegram"
	},
	{
		name: "PostgreSQL",
		use: "Ledgers, CMS, anything that must remember"
	},
	{
		name: "Prisma",
		use: "Schema and migrations we can hand off"
	},
	{
		name: "Discord API",
		use: "Slash commands, economy, moderation"
	},
	{
		name: "Telegram API",
		use: "Menus, shop desks, broadcasts"
	},
	{
		name: "Stripe",
		use: "When money has to move"
	}
];
var ENGAGEMENT = [
	{
	...
	},
	{
		...
		includes: [
			...
			"Backup before touch",
			"Root-cause note",
			"Optional follow-on project"
		]
	}
];
var CAPABILITIES = [
	{
		area: "Marketing sites & storefronts",
		in: true
	},
	{
		area: "Product UI / internal tools",
		in: true
	},
	{
		area: "Discord & Telegram systems",
		in: true
	},
	{
		area: "APIs, workers, Postgres",
		in: true
	},
	{
		area: "Managed hosting & process watch",
		in: true
	},
	{
		area: "SEO and campaign structure",
		in: true
	},
	{
		area: "Native iOS / Android apps",
		in: false
	},
	{
		area: "Print, packaging, fashion",
		in: false
	},
	{
		area: "Paid media spend management at scale",
		in: false
	}
];
var FAQ = [
...
var SERVICES = [
	{
		...
		description: "Full-stack engineering for products that have to ship, scale, and stay maintainable. React and Node on the surface; Postgres, auth, and APIs underneath. We design the architecture before we write the first route.",
		price: 1250,
		billing: "one-off",
		features: [
		...
		deliverables: [
			...
			"Admin or CMS if needed",
			"30-day post-launch patch window"
		],
	...
	},
	{
		...
		description: "Interface design with the restraint of print and the precision of product. We start with type, surface, and a single accent — then build pages that convert without looking like every other agency site.",
		price: 750,
		billing: "one-off",
		features: [
	...
	},
	{
		...
		description: "Reflow, not shrink. We rebuild layouts so type, hit targets, and media behave at 390px first, then up. Cross-browser checks included — Safari, Chrome, Firefox.",
		price: 500,
		billing: "one-off",
		features: [
	...
	},
	{
		...
		description: "A single, tightly written surface with a clear offer, proof, and a form that actually submits. Copy, layout, and tracking in one pass — no carousel of distractions.",
		price: 175,
		billing: "one-off",
		features: [
	...
	},
	{
		...
		description: "A scoped block of engineering time for sites that are already live. Security updates, content swaps, speed work, or the thing you’ve been ignoring. Unused time does not roll — we use the three hours.",
		price: 100,
		billing: "one-off",
	...
	},
	{
		...
		description: "Contracts first: resources, auth, rate limits, errors. Then implementation with tests and docs so the next engineer — including future-you — does not have to reverse-engineer a black box.",
		price: 200,
		billing: "one-off",
		features: [
	...
	},
	{
		...
		description: "A bot is a product. We spec commands, permissions, and failure modes, then ship Discord.js with a real database. Optional AI for support or onboarding. Hosting is separate if you want us to run it.",
		price: 99,
		billing: "one-off",
		features: [
	...
	},
	{
		...
		description: "Telegram is where a lot of communities and shops actually live. We build bots that manage groups, take payments, and talk to your other systems — without the 3 a.m. “bot is down” message.",
		price: 99,
		billing: "one-off",
		features: [
	...
	},
	{
		...
		description: "Bots are not websites. They need a persistent socket, isolated RAM, and a process supervisor that comes back up without you. We run Discord and Telegram workloads on dedicated resources with token-safe config and daily state backups.",
		price: 9.99,
		billing: "monthly",
		features: [
	...
	},
	{
		...
		description: "We take the box, the TLS, the backups, and the updates. Global CDN in front, daily snapshots behind, firewall and patch cadence in between. Built for the sites and apps we design — or for ones you already have.",
		price: 19.99,
		billing: "monthly",
		features: [
	...
	},
	{
		...
		description: "Crawl health, titles, internals, and a keyword map tied to pages you can ship. We do not sell a thousand backlinks. We make the site make sense to Google and to the person who lands.",
		price: 300,
		billing: "one-off",
		features: [
	...
	},
	{
		...
		description: "Strategy, creative, and tracking in one loop. PPC, landing alignment, and conversion events so you can see what the spend did — not a vanity dashboard.",
		price: 250,
		billing: "one-off",
		features: [
			"Campaign strategy",
	...
	},
	{
		...
		description: "We pick the two surfaces that matter, write a month of posts, and set the measurement. Discord and Telegram-native if that is where your people are — not a generic grid.",
		price: 250,
		billing: "one-off",
		features: [
	...
	},
	{
		...
		description: "Product, space, and team coverage with a post pipeline. We shoot for the layouts we already designed so crops, contrast, and color actually fit.",
		price: 400,
		billing: "one-off",
		features: [
...
var PROJECTS = [
	{
		...
		summary: "A contractor-facing web platform for high-performance flooring in South Texas — specs, locales, and a path from inquiry to job.",
		problem: "Decorative and industrial flooring is sold on trust and samples, but the digital presence was a brochure that did not help a GC or property owner take the next step. Specs were scattered. Contact dropped into a void.",
		approach: "We rebuilt the surface as a working platform: systems and use-cases (kitchens, hangars, showrooms) as first-class routes, a cleaner inquiry flow, and a stack a local operator can actually update. Dark, material-led photography instead of clipart concrete.",
		outcome: "A site that reads like the product — dense, engineered, specific. Inquiries route with context (use-case, location, timeline) instead of a blank form.",
		metrics: [
			{
				value: "5",
				label: "Inquiry fields that now matter"
			},
			{
				value: "6w",
	...
	},
	{
		...
		summary: "A Discord bot that does two jobs well: keep the room clean, and give the economy enough depth that people stay.",
		problem: "Most public bots are a pile of commands with no memory. Communities outgrow them the moment they need custom roles, ledgers, or moderation that understands their rules.",
		approach: "Postgres for the ledger and cases. Slash commands with real permission trees. Moderation that logs, expires, and can be appealed. Economy with sinks so inflation does not eat the server in a month.",
		outcome: "The studio’s reference bot — the same skeleton we fork for client servers. Deployed as a product, not a weekend script.",
		metrics: [
			{
				value: "Pg",
				label: "Ledger, not a JSON file"
			},
			{
				value: "RBAC",
	...
	},
	{
		...
		summary: "A retail surface for a smoke shop that needed to feel adult — dark glass, steel, no neon carnival.",
		problem: "The category defaults to loud. The shop wanted a presence that looked like the counter: matte, specific, easy to scan on a phone between errands.",
		approach: "React 19 storefront with a tight catalog, location/hours, and a visual system sampled from the actual fixtures. Type and contrast first; decoration last.",
		outcome: "A site that matches the room. Catalog and visit info without the template smell.",
		metrics: [
			{
				value: "390",
				label: "Designed at phone width first"
			},
			{
				value: "0",
				label: "Stock neon treatments"
			},
			{
				value: "1",
				label: "Visit job: hours + catalog"
			}
		],
	...
	},
	{
		...
		problem: "Quotes, follow-ups, and status lived in three chats and a spreadsheet. Nothing had a source of truth.",
		approach: "A small API and worker set that watches the tools they already use, then writes a single timeline. No new “platform” to log into unless they want it.",
		outcome: "Handoffs stopped falling on the floor. The dashboard is a byproduct — the jobs are the product.",
		metrics: [
			{
				value: "3→1",
				label: "Tools collapsed to a timeline"
			},
			{
				value: "Jobs",
				label: "Product, not the dashboard"
			},
			{
				value: "API",
	...
	},
	{
		...
		approach: "Live ticks over a socket, tabular figures, muted status. No token logos as decoration. Alerts as a list, not confetti.",
		outcome: "An internal board we still use when we need a read, and a pattern we reuse for any live ops surface.",
		metrics: [
			{
				value: "WS",
				label: "Live ticks"
			...
			{
				value: "Tabular",
				label: "Numbers that do not jump"
			},
			{
				value: "Ops",
				label: "Reused as a pattern"
			}
		],
	...
	},
	{
		...
		approach: "A single scrolling night: type as the identity, a quiet city photograph, ticket and lineup as the only jobs of the page.",
		...
		metrics: [
			...
			{
				value: "Type",
				label: "As the identity"
			},
			{
				value: "SMS",
				label: "Shareable without an app"
			}
		],
	...
	},
	{
		...
		tags: ["Web", "Brand"],
		stack: ["React", "Motion"],
		summary: "The follow-up season: a tighter identity system and a page that can host a series, not one date.",
		problem: "One-off event pages rot. The series needed a home that could take the next date without a rebuild.",
		approach: "A season wrapper, archived nights, and a motion language that stays in the type — no particle soup.",
	...
	},
	{
		...
		summary: "The house site. Rebuilt from a zero-stat template into a working studio operating system.",
		problem: "The previous site claimed a cyberpunk aesthetic and then published 0 projects, 0 bots, and 0% satisfaction. Inner pages were thin. Copy repeated itself. Emoji stood in for a design system.",
		approach: "A real content model, honest work, a brief builder, a store, and a support desk. Dark, typographic, no neon. The 3 D’s stay — the execution changes.",
		outcome: "The site you are on. Use it as the proof.",
		metrics: [
			{
				value: "0→8",
				label: "Systems on the record"
			},
			{
				value: "14",
				label: "Priced service lines"
			},
			{
				value: "Core",
				label: "Assistant on the desk"
			}
		],
...
var PRODUCTS = [
	{
		...
		description: "The same Postgres-backed pattern we use on client servers, stripped of private branding. Slash commands, a currency with sinks, inventory, and a moderation-safe admin surface. You bring the token and the rules.",
		includes: [
			"Discord.js source",
	...
	},
	{
		...
		description: "The component language of this studio, packaged. IBM Plex, hairline borders, concentric radii, signal green used once. Buttons, inputs, tabs, data rows, and a command palette shell.",
		includes: [
	...
	},
	{
		...
		description: "A focused script for shops and community desks that need a reliable first reply, a button menu, and a path to a human. Logging included. Extend it or leave it alone.",
		includes: [
	...
	},
	{
		...
		description: "A CSS-only theme for sites that need to go dark without a redesign. Near-black, bone type, sage signal. Works as a starting layer on top of a classless or lightly classed document.",
		includes: [
...
var POSTS = [
	{
		...
		date: "2026-06-21",
		excerpt: "Agents write a lot of the first draft now. Architecture, taste, and the last 20% are still the job.",
		tags: ["AI", "Engineering"],
		reading: "6 min",
		body: [
			{ paragraphs: ["In 2026, an agent can stand up a route, a schema, and a plausible UI before lunch. That is not the same as shipping a product someone will pay for. The studio that treats the agent as a junior who never sleeps, and themselves as the staff engineer, is the one that gets faster without getting sloppy.", "We use agents the way we used to use Stack Overflow and snippets: to collapse boilerplate. We do not let them name the product, pick the type, or invent a third database “just in case.”"] },
			{
				heading: "What actually changed",
				paragraphs: ["The cost of a first implementation dropped. The cost of a wrong abstraction did not. If anything it went up, because you can now generate a wrong abstraction at terrifying speed.", "Review is the skill. Spec is the skill. Knowing when a generated test is asserting the mock, not the behavior — that is the skill."]
			},
			{
				heading: "How we work with it",
				paragraphs: ["Briefs still start on paper. Agents get a bounded task with a definition of done. Humans take the merge. If you want a studio that presses generate and invoices, there are plenty. We are not that."]
			}
		]
	},
	{
		...
		date: "2026-05-12",
		excerpt: "A theme is a loan against uniqueness. Fine for a test. Fatal once you have a real offer.",
		tags: ["Design", "Conversion"],
		reading: "5 min",
		body: [{ paragraphs: ["Templates are honest tools. They get a plumber on the internet by Friday. They also teach the visitor that you are interchangeable, because they have already seen your hero, your three-column features, and your fake 4.9 stars.", "Conversion is not a button color. It is whether the page sounds like the business. Custom design is expensive relative to a theme and cheap relative to a year of looking like everyone else."] }, {
			heading: "When a template is correct",
			paragraphs: ["Validating an offer. A two-week landing test. An internal tool nobody outside the shop will see. We will say so.", "When the brand is the product — a shop, a venue, a specialist contractor — the template is a tax."]
		}]
	},
	{
		...
		date: "2026-04-02",
		excerpt: "Engagement jumps when onboarding, roles, and rewards stop depending on a tired admin.",
		tags: ["Discord", "Bots"],
		reading: "7 min",
		body: [{ paragraphs: ["A public bot is a Swiss Army knife with 400 commands and no memory of your rules. People click it, get a wall, and go back to pinging mods.", "A custom bot is closer to a concierge. It knows the welcome flow, the muted words, the shop, the season pass. It does the jobs that used to sit in a Google Doc titled “please read.”"] }, {
			heading: "The 300% number",
			paragraphs: ["We have seen onboarding completion and weekly chatters move like that when the first ten minutes of a new member’s life are automated well. That is not magic. That is not having to find the right channel while three bots yell.", "If you want a bot that plays music and also runs a bank and also does tickets, we will still ask which of those is the product."]
		}]
	},
	{
		...
		date: "2026-03-18",
		excerpt: "The 2026 dark UI that works looks like print in a dim room — not a LAN party in 2012.",
		tags: ["Design", "Type"],
		reading: "5 min",
		body: [{ paragraphs: ["Cyberpunk as a brand cue aged into a cliché: purple bloom, scanlines on everything, a HUD that fights the content. The sites that still feel expensive in the dark went the other way. Near-black. One metal accent. Type you can sit with.", "We kept the terminal language — status, clocks, mono labels — and threw out the rave. If a glow is doing the job of a hierarchy, the hierarchy failed."] }]
	},
	{
		...
		date: "2026-06-17",
		excerpt: "Slash commands, a real database, Docker, and a process that comes back up. That is the whole talk.",
		tags: ["Discord", "Node.js"],
		reading: "8 min",
		body: [
			{ paragraphs: ["Discord is no longer a gamer VoIP with a bot graveyard. DAOs, shops, and support desks run on it. The engineering bar went up: slash commands, intents you can explain, and state that is not a JSON file on a hobby VPS."] },
			{
				heading: "The stack we actually use",
				paragraphs: ["Node, Discord.js, Postgres. Commands as data. Permissions as a tree, not a boolean. Secrets in the environment, never in the repo. A process manager that restarts on a thrown promise.", "Hosting is a different product from the bot. A sleeping free dyno is not an architecture."]
			},
			{
				heading: "Ship it",
				paragraphs: ["Containerize. Healthcheck the websocket. Log to something you will read. If you do not want to run that, that is what managed bot hosting is for — including ours."]
			}
		]
	},
	{
		...
		date: "2026-02-09",
		excerpt: "Ask to see the runbook, not the moodboard. Ask who answers when it breaks.",
		tags: ["Studio", "Process"],
		reading: "6 min",
		body: [{ paragraphs: ["Most agencies are a sales layer on top of a freelancer network. That is fine if you know it. It is not fine if you thought you bought a studio.", "Ask who will design, who will commit, and who is on the hook the week after launch. Ask for a site they still maintain. Ask what they refused to build last quarter."] }, {
			heading: "A useful test",
			paragraphs: ["Send a messy brief. See if they send back a smaller one. The people who add scope to look busy are the people who will invoice you for looking busy."]
		}]
	},
	{
		...
		date: "2026-01-22",
		excerpt: "Put the marketing site on the edge. Do not put a Discord gateway behind a cold start.",
		tags: ["Infra", "Architecture"],
		reading: "6 min",
		body: [{ paragraphs: ["Serverless is excellent for bursty HTTP. It is a poor home for anything that must hold a connection. We still meet briefs that try to host a bot on a function because “scale.”", "Use the right idle: a small always-on process for the gateway, functions for the webhooks, a database that is not a spreadsheet. Boring, on purpose."] }]
	},
	{
		...
		date: "2025-12-11",
		excerpt: "Version in the URL, errors as objects, and a document that matches production.",
		tags: ["API", "Engineering"],
		reading: "5 min",
		body: [{ paragraphs: ["The future of your API is the same as the past of a good one: a contract. GraphQL or REST is a preference. Undocumented 500s with HTML in the body is a choice.", "We design resources, auth, and rate limits before a single handler. Then we generate the collection you will actually import. That is the whole future."] }]
	}
];
var SEED_REVIEWS = [{
	id: "marcus-t",
	...
	role: "Founder",
	company: "CyberTrade",
	quote: "The platform they designed for us is quiet and fast. Dark, specific, no carnival. Performance held up under a real launch. I would hire them again for the next surface.",
	rating: 5,
	date: "2026-06-14"
}, {
	id: "sarah-j",
	...
	quote: "The custom Discord bot replaced three public bots and a spreadsheet. Onboarding is automatic. Moderation logs are something I can actually read. Engagement went up because people stopped waiting on staff.",
	rating: 5,
	date: "2026-07-22"
}];
var SEED_BOTS = [
	{
		...
		description: "The pattern behind the Economy Bot Template, running here as a sandbox: credits, jobs, a shop, inventory. Slash commands are real. Persistence is not — refresh and the ledger resets. On a client server this writes to Postgres.",
		kind: "economy",
		channel: "Discord",
		welcome: "Ledger online. Sandbox float: 50 credits. Try /work, /shop, /buy coffee, /balance. Chatter is fine — I will not invent your wallet.",
...
Keep replies under 90 words.`,
		starters: [
			...
			"/shop",
			"/daily",
			"How would this run on my Discord?"
		],
		commands: [
			{
				cmd: "/work",
				hint: "Odd job, 8s cooldown"
			},
			{
				cmd: "/balance",
				hint: "Credits on hand"
			},
			{
				cmd: "/daily",
				hint: "Session bonus"
		...
		seoDescription: "Use a live Discord economy bot in the browser: work, shop, inventory. Sandbox demo from N3xUs Konc3pt'z."
	},
	{
		slug: "relay",
		name: "Relay",
		tagline: "A Telegram desk that answers before a human does.",
		description: "Hours, a menu, and a clean handoff. Talk to it like a shop bot: hours, services, how to reach the studio. On a real Telegram it would log and escalate. Here it is the same brain, in the site.",
		kind: "chat",
		channel: "Telegram",
		welcome: "Relay. I cover hours, services, and how to reach the desk. If you have a real job, I will send you to a brief.",
...
Keep replies under 90 words.`,
		...
		commands: [
			{
				cmd: "hours",
				hint: "When the desk is on"
			},
			{
				cmd: "menu",
				hint: "What the studio sells"
			},
			{
				cmd: "human",
				hint: "Handoff to a brief"
			}
		],
		...
		seoDescription: "Talk to a live Telegram-style auto-responder from N3xUs Konc3pt'z. Hours, services, and a path to a human."
	},
	{
		slug: "house",
		name: "House",
		tagline: "A Discord casino you can actually lose chips in.",
		description: "Slots, coinflip, dice, roulette, blackjack — the same command surface we ship on client servers. Sandbox chips, real house edge. Refresh resets the cage. On Discord this writes to Postgres and pays a configured currency.",
		kind: "casino",
		channel: "Discord",
		welcome: "House is open. Sandbox float: 200 chips. Not money. /slots 10, /flip heads 25, /roulette red 20, /bj 25 then /hit or /stand. /daily if you burn the stack.",
...
Keep replies under 90 words.`,
		...
		seoDescription: "Play a live Discord casino bot in the browser: slots, coinflip, roulette, blackjack. Sandbox chips from N3xUs Konc3pt'z. Not real money."
	}
];
var NAV = [
	{
...
function seedSite() {
	return {
		settings: {
			...
			seoDescription: "South Texas digital studio for custom websites, apps, Discord and Telegram bots, SEO, and managed hosting. Design, development, and deployment from one desk.",
			seoKeywords: "web design, web development, discord bots, telegram bots, managed hosting, SEO, South Texas, custom websites, N3xUs Konc3ptz",
			city: "South Texas",
			region: "Texas",
			country: "US",
			twitterHandle: "",
...
async function ensureSeeded() {
	g.__cmsSeeded__ ??= (async () => {
		...
		if (((await sql`select count(*)::int as n from settings`)[0]?.n ?? 0) === 0) {
			await insertSeed(sql);
			return;
		}
	})().catch((err) => {
		g.__cmsSeeded__ = void 0;
		throw err;
	});
	await g.__cmsSeeded__;
	g.__cmsBotsV2__ ??= (async () => {
		await insertBots(await getSql(), seedSite().bots);
	})().catch((err) => {
		g.__cmsBotsV2__ = void 0;
		throw err;
	});
	await g.__cmsBotsV2__;
	await refreshStudioCopy();
}
async function refreshStudioCopy() {
	g.__cmsCopyV4__ ??= (async () => {
		const sql = await getSql();
		const row = (await sql.query("select founder_bio, genesis from settings where id = $1", ["studio"]))[0];
		if (!row) return;
		const bio = String(row.founder_bio ?? "");
		const genesis = String(row.genesis ?? "");
		if (bio === "Juan runs N3xUs Konc3pt'z as a small digital studio: design, engineering, and the machines that keep both online. He writes the journal, ships the bots, and still does the type pass himself." || genesis === "The studio was started to refuse the usual split: a designer who never deploys, an engineer who inherits a moodboard, a host nobody owns. We take the 3 D’s as one job. Ecommerce, Discord, Telegram, CRMs, APIs — if it has to live on a network, it is in scope.") await sql.query("update settings set founder_bio = $1, genesis = $2, updated_at = now() where id = $3", [
			FOUNDER.bio,
			SEED_GENESIS,
			"studio"
		]);
		for (let i = 0; i < PRINCIPLES.length; i++) {
			...
			await sql.query("update principles set title = $1, body = $2 where id = $3", [
				p.title,
				p.body,
				`pr-${i + 1}`
			]);
		...
		for (let i = 0; i < PIPELINE.length; i++) {
			const p = PIPELINE[i];
			await sql.query("update pipeline set title = $1, body = $2 where id = $3", [
				p.title,
				p.body,
				`pl-${i + 1}`
			]);
		...
		for (let i = 0; i < FAQ.length; i++) {
			const f = FAQ[i];
			await sql.query("update faqs set question = $1, answer = $2 where id = $3", [
				f.q,
				f.a,
				`faq-${i + 1}`
			]);
...
async function insertSeed(sql) {
	...
	await sql.query(`insert into settings (
	...
	for (const f of seed.faqs) await sql.query(`insert into faqs (id, question, answer, sort_order) values ($1,$2,$3,$4)`, [
		f.id,
		f.question,
		f.answer,
		f.sortOrder
	]);
	for (const p of seed.principles) await sql.query(`insert into principles (id, num, title, body, sort_order) values ($1,$2,$3,$4,$5)`, [
		p.id,
		p.num,
		p.title,
		p.body,
	...
	for (const p of seed.pipeline) await sql.query(`insert into pipeline (id, num, title, body, sort_order) values ($1,$2,$3,$4,$5)`, [
		p.id,
		p.num,
		p.title,
		p.body,
	...
	for (const e of seed.engagements) await sql.query(`insert into engagements (id, name, range_label, body, includes, sort_order) values ($1,$2,$3,$4,$5::jsonb,$6)`, [
		e.id,
		e.name,
		e.rangeLabel,
		e.body,
	...
	for (const c of seed.capabilities) await sql.query(`insert into capabilities (id, area, in_scope, sort_order) values ($1,$2,$3,$4)`, [
		c.id,
		c.area,
		c.inScope,
		c.sortOrder
	]);
	for (const svc of seed.services) await sql.query(`insert into services (
	...
	for (const p of seed.projects) await sql.query(`insert into projects (
	...
	for (const p of seed.products) await sql.query(`insert into products (
	...
	for (const p of seed.posts) await sql.query(`insert into posts (
	...
	for (const r of seed.reviews) await sql.query(`insert into reviews (id, name, role, company, quote, rating, date, published) values ($1,$2,$3,$4,$5,$6,$7,$8)`, [
		r.id,
		r.name,
		r.role,
		r.company,
...
async function insertBots(sql, bots) {
	for (const b of bots) await sql.query(`insert into bots (
...
function mapSettings(row) {
	return {
		...
		indexable: row.indexable === void 0 ? true : bool(row.indexable)
	};
}
...
function mapService(row) {
	return {
		...
		hoursNote: row.hours_note ? String(row.hours_note) : void 0,
...
async function readSite(publishedOnly = true) {
	await ensureSeeded();
	const sql = await getSql();
	const pub = publishedOnly ? "where published = true" : "";
	const [settingsRows, faqs, principles, pipeline, engagements, capabilities, services, projects, products, posts, reviews, bots] = await Promise.all([
		sql.query("select * from settings where id = $1", ["studio"]),
		sql.query("select * from faqs order by sort_order"),
		sql.query("select * from principles order by sort_order"),
		sql.query("select * from pipeline order by sort_order"),
		sql.query("select * from engagements order by sort_order"),
		sql.query("select * from capabilities order by sort_order"),
		sql.query(`select * from services ${pub} order by sort_order`),
		sql.query(`select * from projects ${pub} order by sort_order`),
		sql.query(`select * from products ${pub} order by sort_order`),
		sql.query(`select * from posts ${pub} order by date desc`),
		sql.query(`select * from reviews ${publishedOnly ? "where published = true" : ""} order by date desc`),
		sql.query(`select * from bots ${pub} order by sort_order`)
	]);
	return {
		settings: settingsRows[0] ? mapSettings(settingsRows[0]) : seedSite().settings,
		faqs: faqs.map((r) => ({
			id: String(r.id),
...
async function readInbox() {
	...
	const [briefs, messages, orders] = await Promise.all([
		sql.query("select * from briefs order by created_at desc"),
		sql.query("select * from messages order by created_at desc"),
		sql.query("select * from orders order by created_at desc")
	]);
	return {
		briefs: briefs.map((r) => ({
			id: String(r.id),
...
async function listOwners() {
	return (await (await getSql()).query("select user_id, email from owners order by created_at")).map((r) => ({
		userId: r.user_id,
		email: r.email
	}));
}
async function claimOrRequireOwner(userId, email) {
	const sql = await getSql();
	const existing = await sql.query("select user_id from owners");
	if (existing.length > 0) {
		if (!existing.some((r) => r.user_id === userId)) throw new Error("This desk already has an owner.");
		return;
	}
	const callerEmail = email ?? await lookupAuthEmail(sql, userId);
	const gate = authConfigured ? ownerEmail() : void 0;
	if (gate) {
		if (!callerEmail || callerEmail.toLowerCase() !== gate) throw new Error("Owner claim rejected: this account is not the configured desk owner.");
	}
	await sql.query("insert into owners (user_id, email) values ($1,$2) on conflict (user_id) do nothing", [userId, callerEmail ?? null]);
	if (!(await sql.query("select user_id from owners")).some((r) => r.user_id === userId)) throw new Error("This desk already has an owner.");
}
/** Look up a Better Auth user's email by id. Fails closed: returns null on error. */
async function lookupAuthEmail(sql, userId) {
	try {
		return (await sql.query("select \"email\" from \"user\" where \"id\" = $1", [userId]))[0]?.email ?? null;
	} catch {
		return null;
	}
}
async function addOwner(userId, email) {
	await (await getSql()).query("insert into owners (user_id, email) values ($1,$2) on conflict (user_id) do update set email = excluded.email", [userId, email]);
}
async function removeOwner(userId, actorId) {
	if (userId === actorId) throw new Error("You cannot remove yourself.");
	const sql = await getSql();
	if (((await sql.query("select count(*)::int as n from owners"))[0]?.n ?? 0) <= 1) throw new Error("At least one owner is required.");
	await sql.query("delete from owners where user_id = $1", [userId]);
}
async function saveSettings(s) {
	await (await getSql()).query(`update settings set
...
async function replaceSimple(table, rows) {
	const sql = await getSql();
	await sql.query(`delete from ${table}`);
	if (table === "faqs") for (const raw of rows) await sql.query(`insert into faqs (id, question, answer, sort_order) values ($1,$2,$3,$4)`, [
		raw.id || uid("faq"),
		raw.question,
		raw.answer,
		raw.sortOrder
	]);
	else if (table === "principles") for (const raw of rows) await sql.query(`insert into principles (id, num, title, body, sort_order) values ($1,$2,$3,$4,$5)`, [
		raw.id || uid("pr"),
		raw.num,
		raw.title,
		raw.body,
	...
	else if (table === "pipeline") for (const raw of rows) await sql.query(`insert into pipeline (id, num, title, body, sort_order) values ($1,$2,$3,$4,$5)`, [
		raw.id || uid("pl"),
		raw.num,
		raw.title,
		raw.body,
	...
	else if (table === "engagements") for (const raw of rows) await sql.query(`insert into engagements (id, name, range_label, body, includes, sort_order) values ($1,$2,$3,$4,$5::jsonb,$6)`, [
		raw.id || uid("eg"),
		raw.name,
		raw.rangeLabel,
		raw.body,
	...
	else for (const raw of rows) await sql.query(`insert into capabilities (id, area, in_scope, sort_order) values ($1,$2,$3,$4)`, [
		raw.id || uid("cap"),
		raw.area,
		raw.inScope,
		raw.sortOrder
...
async function upsertService(svc) {
	await (await getSql()).query(`insert into services (
      slug, name, group_name, summary, description, price, billing, hours_note,
...
async function deleteBySlug(table, slug) {
	await (await getSql()).query(`delete from ${table} where slug = $1`, [slug]);
}
async function upsertProject(p) {
	await (await getSql()).query(`insert into projects (
      slug, title, client, kind, year, duration, role, tags, stack, summary,
      problem, approach, outcome, metrics, image, featured, published, sort_order,
...
async function upsertProduct(p) {
	await (await getSql()).query(`insert into products (
...
async function upsertPost(p) {
	await (await getSql()).query(`insert into posts (
...
async function upsertBot(b) {
	await (await getSql()).query(`insert into bots (
      slug, name, tagline, description, kind, channel, welcome, persona,
...
async function getPublishedBot(slug) {
	await ensureSeeded();
	const rows = await (await getSql()).query("select * from bots where slug = $1 and published = true", [slug]);
	return rows[0] ? mapBot(rows[0]) : null;
}
async function upsertReview(r) {
	await (await getSql()).query(`insert into reviews (id, name, role, company, quote, rating, date, published)
...
async function deleteReview(id) {
	await (await getSql()).query("delete from reviews where id = $1", [id]);
}
async function insertBrief(input) {
	await ensureSeeded();
	const sql = await getSql();
	const id = uid("brf");
	await sql.query(`insert into briefs (id, name, email, channel, groups, budget, timeline, notes, status)
...
async function insertMessage(input) {
	...
	const sql = await getSql();
	const id = uid("msg");
	await sql.query(`insert into messages (id, name, email, topic, body, status) values ($1,$2,$3,$4,$5,'new')`, [
		id,
		input.name,
		input.email,
		input.topic,
...
async function insertOrder(input) {
	...
	const sql = await getSql();
	const id = uid("ord");
	await sql.query(`insert into orders (id, email, channel, total, items, status) values ($1,$2,$3,$4,$5::jsonb,'queued')`, [
		id,
		input.email,
		input.channel,
		input.total,
...
async function insertReview(input) {
	...
	await sql.query(`insert into reviews (id, name, role, company, quote, rating, date, published)
...
async function setRowStatus(table, id, status) {
	await (await getSql()).query(`update ${table} set status = $1 where id = $2`, [status, id]);
}
//#endregion
export { saveSettings as A, upsertProduct as B, parseCommands as C, readSite as D, readInbox as E, setRowStatus as F, upsertReview as H, slugify as I, uid as L, serializeCommands as M, serializeMetrics as N, removeOwner as O, serializePostBody as P, upsertBot as R, listOwners as S, parsePostBody as T, upsertService as U, upsertProject as V, insertBrief as _, STACK_NOTES as a, insertReview as b, claimOrRequireOwner as c, deleteBySlug as d, deleteReview as f, hasGateSessionMarker as g, getPublishedBot as h, PACKAGE_VERSION as i, seedSite as j, replaceSimple as k, cn as l, getBaseURL as m, GROK_PROVIDERS as n, addOwner as o, formatUsd as p, NAV as r, auth as s, GENERIC_OAUTH_ERROR_CODES as t, db_CgZiGDRe_exports as u, insertMessage as v, parseMetrics as w, lines as x, insertOrder as y, upsertPost as z };
